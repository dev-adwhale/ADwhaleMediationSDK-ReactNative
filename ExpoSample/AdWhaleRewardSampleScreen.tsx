// example/src/AdWhaleRewardSampleScreen.tsx
...
const AdWhaleRewardSampleScreen: React.FC<Props> = ({onBack}) => {
    ...
    const subs = AdWhaleRewardAd.addEventListeners({
      ...
      onLoadFailed: (e: AdWhaleRewardErrorEvent) => {
        setState(prev => ({
          ...prev,
          isRewardAdLoaded: false,
          debugInfo: `보상형 광고 로드 실패: ${e.statusCode} - ${e.message}\n`,
        }));
      },
      onShowed: () => {
        setState(prev => ({
          ...prev,
          debugInfo: '보상형 광고 표시됨\n',
        }));
      },
      onShowFailed: (e: AdWhaleRewardErrorEvent) => {
        setState(prev => ({
          ...prev,
          isRewardAdLoaded: false,
          debugInfo: `보상형 광고 표시 실패: ${e.statusCode} - ${e.message}\n`,
        }));
      },
      onDismissed: () => {
        setState(prev => ({
      ...
      onClicked: () => {
        setState(prev => ({
          ...prev,
          debugInfo: '보상형 광고 클릭됨\n',
        }));
      },
      onUserRewarded: (e: AdWhaleRewardUserRewardedEvent) => {
        setState(prev => ({
          ...prev,
          debugInfo: `보상 지급됨: ${e.type} - ${e.amount}\n`,
        }));
      },
    });
  ...
  const handleCheckStatus = async () => {
    console.log('[AdWhaleReward] handleCheckStatus called');
    try {
      setState(prev => ({...prev, debugInfo: 'Status 확인 중...\n'}));
      const status = await AdWhaleMediationAds.getConsentStatus();
      console.log('[AdWhaleReward] Status result:', status);
      const msg =
      ...
      console.error('[AdWhaleReward] Get Status Error:', e);
      setState(prev => ({
        ...prev,
  ...
  const handleResetGdpr = () => {
    console.log('[AdWhaleReward] handleResetGdpr called');
    try {
      ...
      console.error('[AdWhaleReward] Reset Error:', e);
      setState(prev => ({
        ...prev,
  ...
  const handleSetGdpr = (consent: boolean) => {
    console.log('[AdWhaleReward] handleSetGdpr called:', consent);
    try {
      ...
      console.error('[AdWhaleReward] SetGdpr Error:', e);
      setState(prev => ({
        ...prev,
  ...
  const handleAdShow = () => {
    if (!state.isRewardAdLoaded) return;
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.container}>
        <ScrollView style={styles.scrollView}>
          <Text style={styles.title}>보상형 광고 (TypeScript)</Text>
              ...
              <Switch
                ...
                onValueChange={v => {
                  ...
                  try {
                    ...
                    console.log('[AdWhaleReward] getLogLevel() 결과:', logLevel);
                  } catch (error) {
                    console.error('[AdWhaleReward] getLogLevel() 실패:', error);
                  }
                }}
              />
            </View>
...
export default AdWhaleRewardSampleScreen;
