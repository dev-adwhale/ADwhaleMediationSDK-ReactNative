 ...
 * Flutter transition_popup_ad_test_page.dart 와 1:1 매핑된 화면입니다.
 *
 * Android 전용: AdWhaleTransitionPopupAd (앱 전환 팝업 광고)
 * iOS에서는 미지원 안내만 표시합니다.
 *
 * 올바른 API 사용:
 *  - AdWhaleTransitionPopupAd.addEventListeners({...})  (useEffect, Android 전용, 언마운트 시 정리)
...
const AdWhaleTransitionPopupSampleScreen: React.FC<Props> = ({onBack}) => {
  const [state, setState] = useState<State>({
    loggerEnabled: false,
    coppaEnabled: false,
    placementUid: AdConfig.transitionPopupPlacementUid,
    placementName: '',
    region: '',
    gcoder: undefined,
    debugInfo:
  ...
  useEffect(() => {
    ...
    const subs = AdWhaleTransitionPopupAd.addEventListeners({
      ...
      onShowed: () => {
        setState(prev => ({...prev, debugInfo: 'Transition 팝업 표시됨\n'}));
      },
      ...
      <View style={styles.container}>
        <ScrollView style={styles.scrollView}>
          <Text style={styles.title}>
            앱 전환 팝업 (Transition Popup, Android)
          </Text>

          {Platform.OS !== 'android' && (
            <View style={styles.warnBox}>
              <Text style={styles.warnText}>이 샘플은 Android 전용입니다.</Text>
            </View>
          )}
...
export default AdWhaleTransitionPopupSampleScreen;
